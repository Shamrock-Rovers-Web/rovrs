export interface OffsitePreviewOptions {
  slug: string;
  destination: string;
  domain: string;
}
 ...
 */
export function renderOffsitePreview(options: OffsitePreviewOptions): string {
...
<head>
  ...
  <title>Preview - ${slug} | rov.rs</title>
  <style>
    * {
      margin: 0;
      padding: 0;
...
<body>
  <div class="container">
    ...
    <div class="security-note">
      ⚠️ This link directs to ${domain}, which is not part of rov.rs
    </div>

    <div class="preview-info">
      <div class="preview-domain">${destDomain}</div>
      <div class="preview-url">${destination}</div>
      <div style="margin-top: 8px; font-size: 14px; color: #888;">
        Short link: <span class="slug">rov.rs/${slug}</span>
      </div>
    </div>
